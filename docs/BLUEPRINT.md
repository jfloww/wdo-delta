...
> Version: 0.2
> Status: Phased delivery baseline for the deterministic engine
> Last updated: 2026-08-17

**Scope note (2026-08-21).** This document designs the *deterministic engine* — the offer and
relocation decision core — and was written when that engine was the whole product. It is still the
authoritative record of those decisions: cost ownership, period handling, rounding policy, the
tax-model port, break-even semantics, and the money-across-the-boundary rule.

It predates the wider Personal Finance Copilot framing, in which the offer comparison is one
scenario the engine answers rather than the product itself. It therefore does not cover CSV ingest,
the categorisation taxonomy, the evaluation harness, or the LLM client. Those are described in the
[README](../README.md). Where this document says "OfferDelta", read it as the name of the
calculation core; see the README's note on naming.
